...
import { isUnset, isSet, decodeValue, encodeValue, getProp, getMainDomain } from './utilities'
...
export default class Storage {
  ...
  setCookie (key, value, options = {}) {
    ...
    let domain;
    if (process.client) {
      domain = getMainDomain(window.location.host);
    } else {
      domain = '.' + getMainDomain(this.ctx.req.headers.host);
    }
    if (!domain.startsWith('localhost')) {
      domain = '.' + domain;
    }
    console.log('fallback cookie domain:', domain);
    
    const _key = this.options.cookie.prefix + key
    const _options = Object.assign({ domain }, this.options.cookie.options, options)
    ...
    if (process.client) {
      ...
      console.log('nuxt.auth, headers', this.ctx.req.headers);
